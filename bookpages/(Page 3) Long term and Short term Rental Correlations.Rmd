...
## Page 3 Long vs Short Term Rental Correlations
### Introduction

This page will look to find if there are predictive factors ofr short term vs long term rentals on the Airbnb platform
...
The graphs first peak is at 10 days at about 10000. Roughly 17500 properties out of the total fall below the 90 day mark, which is the cut off point for short term rental. The next peak is around 1 year with approximately 9500 and the final and largest peak is around 3 years roughly being 38,000. This corroborates the claims that Airbnb and the short term rental market is being used as an alternative to the housing rental market.
...
### Investigating the 1 year and 3 year let duration price spread


The graph above clearly highlights the improper use, but it also suggests that hosts listing for durations of 1 year and 3 years, were charging more than those at any other duration. 

However the graph deosnt appear to suggest that all the listings that have longer durations charge more. It only appears that it sits at 365 and 1125. 

Thus, we did some prelimenary analysis and investigation to see if this really is the case. 

```{python}
# Filtering data to properties with at 365 and 1125
...
# Obtaining data around the year and three year point for comparison data 
...
#Difference in the data on average, the means of 1 year 1125 days and the around these numbers 
...
This brief analysis suggests that the listings for a year are more expensive than those around it confirming our graph obseravation. 

However the calculations also reveal that the propeties listed for three years were listed at substantially lower prices on average than those around the 3 year mark. 
...
To do this we employed the use of quartiles to represent how the data is spread within each group.
...
<!-- #region -->
The two representations demonstrate how the data is spread within each group - year one, around year one etc. Therefore we are able to get a better understanding the results for 3 years and large differences in the mean for 1 year 3 years. 

First, lets consider the upper quartile. It appears that three quarters of all results in each group are within a small price range however the upper quartile reveals a much larger spread of data. The larger values in this quartile appear siginificantly greater than the results in the other quartiles and therefore may have the ability to influence the mean. However, this is not true for all data. You would expect the listing groups with a greater number of results aka. 1 year and 3 years to have a higher number of these large results by chance. This is not the case and although 3 years contains the most values it has much similar values than one year or even around 1 year.  
